# List of google group emails are granted the roles# List of google group emails are granted the roles#### GRANT USER ACCESS THE PRODUCTION FOLDERS ####
data "terraform_remote_state" "alpha" {
  backend = "gcs"
  config = {
    bucket = "tiliter-terraform-iac"
...
locals {
  roles = {
    admin = "projects/${local.project_id}/roles/${data.terraform_remote_state.alpha.outputs.admin_custom_role}"
    dev   = "projects/${local.project_id}/roles/${data.terraform_remote_state.alpha.outputs.dev_custom_role}"
    read  = "projects/${local.project_id}/roles/${data.terraform_remote_state.alpha.outputs.read_custom_role}"
  }
}
