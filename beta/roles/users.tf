# List of google group emails are granted the roles#### GRANT USER ACCESS THE PRODUCTION FOLDERS ####
data "terraform_remote_state" "beta" {
  backend = "gcs"
  config = {
...
locals {
  roles = {
    admin = "projects/${local.project_id}/roles/${data.terraform_remote_state.beta.outputs.admin_custom_role}"
    dev   = "projects/${local.project_id}/roles/${data.terraform_remote_state.beta.outputs.dev_custom_role}"
    read  = "projects/${local.project_id}/roles/${data.terraform_remote_state.beta.outputs.read_custom_role}"
  }
}
...
resource "google_project_iam_binding" "iam_binding" {
  for_each = { for idx, perm in local.access_permission_lists : 
